...
local function AddItem(source, item, amount, slot, info, created)
	...
	if not itemInfo then 
		QBCore.Functions.Notify(source, "Item does not exist "..item, 'error')
		return false
	end
	if not created then
...
local function SetInventory(source, items)
	...
	TriggerEvent('qb-log:server:CreateLog', 'playerinventoryset', 'SetInventory', 'blue', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** items set: ' .. json.encode(items))
end
...
local function CreateNewDrop(source, fromSlot, toSlot, itemAmount, created)
	...
	if RemoveItem(source, itemData.name, itemAmount, itemData.slot) then
		...
		TriggerClientEvent("inventory:client:DropItemAnim", source, dropId)
...
local function OpenInventory(name, id, other, origin)
	...
	if ply.state.inv_busy then return QBCore.Functions.Notify(src, "Inventory is busy", 'error') end
	if name and id then
		local secondInv = {}
...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
	...
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "trunk" then
		...
		if fromItemData and fromItemData.amount >= fromAmount then
            ...
            QBCore.Functions.Notify(src,"Item not exist", "error")
		end
...
RegisterServerEvent("inventory:server:GiveItem", function(target, name, amount, slot)
	...
	-- local tp = Player(target)
	-- if tp.state.inv_busy then
	-- 	return QBCore.Functions.Notify(src, "Inventory is busy", 'error')
	-- end
...
QBCore.Functions.CreateCallback('inventory:server:ConvertQuality', function(source, cb, inventory, other)
    ...
    for _, item in pairs(inventory) do
		...
		elseif QBCore.Shared.Items[item.name:lower()]["decay"] ~= nil or QBCore.Shared.Items[item.name:lower()]["decay"] ~= 0 then
			if QBCore.Shared.Items[item.name:lower()]["usedecay"] and QBCore.Shared.Items[item.name:lower()]["useable"] then
				if item.info.created then
					item.created = item.info.created
				else
					item.created = os.time()
					local info = {created = os.time()}
					item.info = info
				end
			end
        end
...
RegisterNetEvent('inventory:server:dropPlayer', function(reason)	
	local src = source	
	local Player = QBCore.Functions.GetPlayer(src)	
	TriggerEvent("qb-log:server:CreateLog", "inventryblacklistedkey", "Player Kicked - (.) Key pressed", "red", "**".. GetPlayerName(source) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..source.."*) ** Kicked - "..reason.."**, \n\n("..(QBCore.Functions.GetIdentifier(src, 'discord') or 'undefined') .." |  ||"  ..(QBCore.Functions.GetIdentifier(src, 'ip') or 'undefined') ..  "|| | " ..(QBCore.Functions.GetIdentifier(src, 'license') or 'undefined') .." | " ..(QBCore.Functions.GetIdentifier(src, 'steam') or 'undefined') ..")")	
	DropPlayer(src,reason)	
end)	
RegisterNetEvent('inventory:server:sendBlacklistedkeyLog', function(reason)	
	local src = source	
	local Player = QBCore.Functions.GetPlayer(src)	
	TriggerEvent("qb-log:server:CreateLog", "inventryblacklistedkey", "(.) Key pressed", "yellow", "**".. GetPlayerName(source) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..source.."*) **"..reason.."**, \n\n("..(QBCore.Functions.GetIdentifier(src, 'discord') or 'undefined') .." |  ||"  ..(QBCore.Functions.GetIdentifier(src, 'ip') or 'undefined') ..  "|| | " ..(QBCore.Functions.GetIdentifier(src, 'license') or 'undefined') .." | " ..(QBCore.Functions.GetIdentifier(src, 'steam') or 'undefined') ..")")	
end)
